...
class Licenses extends Trongate {
    ...
    function _get_drivers_options($selected_key) {

        if (($selected_key == '') || ($selected_key == 0) || ($selected_key == '0')) {
            $options[''] = 'Select Driver...';
        }
        
        $sql = "select * from drivers order by last_name";
        $rows = $this->model->query($sql, 'object');

        foreach ($rows as $row) {
            $row_desc = $row->last_name;;
    ...
    function create() {
        ...
        $data['drivers_options'] = $this->_get_drivers_options($data['drivers_id']);
    ...
    function manage() {
        ...
        $pagination_data['record_name_plural'] = 'licenses';
    ...
    function _get_data_from_db($update_id) {
        ...
        if ($licenses == false) {
            ...
            $data['drivers_id'] = $licenses->drivers_id;
            return $data;
        }
    }
    ...
    function _get_data_from_post() {
        ...
        $data['drivers_id'] = input('drivers_id', true);
        return $data;
    }
